...
export default function Enter() {
    ...
    return <>{user ? isRegistered ? <Home/> : <Register student={userStudentType}/> : <Login/>}</>
}
...
const userExists = async ({user, studentType, name, username}: {user: User | null | undefined, studentType: boolean, name: string, username: string}) => {
    ...
    if (!snap.exists() && !usernameSnap.exists()) {
        ...
        batch.set(ref, { username: username.toLowerCase(), studentType: studentType, name: name, lastName: 'Escriva', photoURL: user?.photoURL, displayName: user?.displayName});
        await batch.commit();
    }
}
                            ...
                            <FormControl>
                                {!student && (
                                    <>
                                        ...
                                        <Input id='username' type='text' marginBottom={"4"} onChange={(e) => {setUsername(e.currentTarget.value)}}/>
                                        <FormLabel htmlFor="name">Nombre Empresa</FormLabel>
                                        <Input id="name" type="text" marginBottom={"4"} onChange={(e) => {setValue(e.currentTarget.value)}}/>
                                        <FormLabel htmlFor="cif">CIF</FormLabel>
                                        <Input id="cif" type="text" marginBottom={"4"}/>
                                        <FormLabel htmlFor="employers">Número de Empleados</FormLabel>
                                ...
                                <Center>
                                    <Button type='submit' colorScheme='teal' onClick={() => {userExists({user, studentType: student, name: value, username})}}>Register</Button>
                                </Center>
                            </FormControl>
                        </Stack>
                    </Stack>
